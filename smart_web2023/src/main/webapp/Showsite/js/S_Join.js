    ...
    function getValue() {
        ...
        if (idinput.value.length == 0) {
            ...
            idinput.focus();
            return false; //아이디 입력
        } else if (!regExp_id.test(idinput.value)) {
            alert("아이디는 8글자 이상 10글자 이하로 숫자를 1개 이상 포함해야 합니다");
            idinput.focus();
            //비밀번호
            ...
            pwinput.focus();
            return false; //비밀번호 입력
        } else if (!regExp_pw.test(pwinput.value)) {
            alert("비밀번호는 8자~20자리의 영문, 숫자, 특수문자 중 2가지 조합하여야 합니다.");
            pwinput.focus();
            return false;
            //비밀번호 확인
        } else if (pwinput2.value.length == 0) {
        ...
        //이름, 휴대폰 번호, 이메일
            ...
            nameinput.focus(); //이름 입력
        } else if (phone.value.length == 0) {
            alert("전화번호가 입력되지 않았습니다");
            phone.focus(); //휴대폰 번호 입력
        } else if (!regExp_phone.test(phone.value)) {
            alert("전화번호는 010-숫자4자리-숫자4자리 로 입력해야 합니다");
            phone.focus(); //휴대폰 번호 입력
        } else if (emailinput.value.length == 0) {
            alert("이메일이 입력되지 않았습니다");
            emailinput.focus(); //이메일 입력
        } else if (emailinput2.value.length == 0) {
            alert("이메일이 입력되지 않았습니다");
            emailinput2.focus(); //이메일 입력
        } else if (!myCheckbox.checked) {
            alert("이용약관에 동의하지 않았습니다.");
            myCheckbox.focus(); //이메일 입력
        } else {
            valid = true;
        }
        if (!valid) {
