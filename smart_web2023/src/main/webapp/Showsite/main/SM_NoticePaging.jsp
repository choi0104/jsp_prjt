...
<c:choose>
	<c:when test="${not empty searchWord}"><!-- 검색어가 있는 경우  -->
		...
		<c:if test="${Notice_pageNum > 5}">
			...
			&nbsp;
			<a href="S_Manager.jsp?
			Notice_pageNum=${(Notice_pageBlock-2)*pages_pageBlock+1}&	Event_pageNum=${Event_pageNum}&News_pageNum=${News_pageNum}&
			Notice_pageBlock=${Notice_pageBlock-1}&Event_pageBlock=${Event_pageBlock}&News_pageBlock=${News_pageBlock}&
			searchField=${searchField}&searchWord=${searchWord}">이전</a>
		</c:if>
		
		<!-- 1. 총 페이지번호와 페이지블록을 연계하여 페이지번호 출력 --> 
		<c:forEach var="p" begin="${(Notice_pageBlock - 1)*pages_pageBlock +1}" end="${Notice_pageBlock*pages_pageBlock}">
		...
		<!-- 2. 총 페이지번호가 페이지블록을 초과하는 경우 '다음'과 맨 마지막페이지 기호(>>) 출력 -->
		<c:if test="${Notice_total_pageNum > (Notice_pageBlock*pages_pageBlock)}">
			<a href="S_Manager.jsp?
	...
	<c:otherwise><!-- 검색어가 없는 경우  -->
		...
		<c:if test="${Notice_pageNum > 5}">
			<a href="S_Manager.jsp?
			Notice_pageNum=1&Event_pageNum=${Event_pageNum}&News_pageNum=${News_pageNum}&
			Notice_pageBlock=1&Event_pageBlock=${Event_pageBlock}&News_pageBlock=${News_pageBlock}">&lt;&lt;</a>
			&nbsp;
			<a href="S_Manager.jsp?
			Notice_pageNum=${(Notice_pageBlock-2)*pages_pageBlock+1}&Event_pageNum=${Event_pageNum}&News_pageNum=${News_pageNum}&
			Notice_pageBlock=${Notice_pageBlock-1}&Event_pageBlock=${Event_pageBlock}&News_pageBlock=${News_pageBlock}">이전</a>
		</c:if>
		...
		<c:if test="${Notice_total_pageNum > (Notice_pageBlock*pages_pageBlock)}">
			<a href="S_Manager.jsp?
			Notice_pageNum=${(Notice_pageBlock*pages_pageBlock)+1}&Event_pageNum=${Event_pageNum}&News_pageNum=${News_pageNum}&
			Notice_pageBlock=${Notice_pageBlock + 1}&Event_pageBlock=${Event_pageBlock}&News_pageBlock=${News_pageBlock}">다음</a>
			&nbsp;
			<a href="S_Manager.jsp?
			Notice_pageNum=${Notice_total_pageNum}&Event_pageNum=${Event_pageNum}&News_pageNum=${News_pageNum}&
			Notice_pageBlock=${Notice_last_pageBlock}&Event_pageBlock=${Event_pageBlock}&News_pageBlock=${News_pageBlock}">&gt;&gt;</a>
		</c:if>
	</c:otherwise>
</c:choose>
