...
public class S_DBconnection {
	...
	protected Connection S_conn;
	protected PreparedStatement S_pstmt;
	protected ResultSet S_rs;
	
	//기본생성자에 연결객체가 생성되도록 정의함
	public S_DBconnection() {
		try {
			Class.forName("oracle.jdbc.OracleDriver");
			System.out.println("드라이버 로딩 성공");
			String url = "jdbc:oracle:thin:@192.168.0.50:1521:xe";
			S_conn = DriverManager.getConnection(url, "smart_dev", "1234");
	...
	public void close() {
		try {
			if(S_conn != null) {S_conn.close();}
			if(S_pstmt != null) {S_pstmt.close();}
			if(S_rs != null) {S_rs.close();}
