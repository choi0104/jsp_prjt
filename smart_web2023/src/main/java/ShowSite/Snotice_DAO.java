...
public class Snotice_DAO extends S_DBconnection {
	
	public int insert(Snotice_DTO dto) {
		int result = 0;//실패 시의 값
		
		try {
			if(dto.getOriginfile_name() != null) {
				//파일 업로드가 이루어질 경우의 insert문
				String sql = "insert into SNOTICE_BOARD (b_idx, s_idx, writer, title, content, originfile_name, savefile_name) "
				...
				String sql = "insert into SNOTICE_BOARD (b_idx, s_idx, writer, title, content) "
						     + " values(board_seq.nextval, ?,?,?,?)";
	...
	//모든 게시글 조회
	public ArrayList<Snotice_DTO> getBoards(){
		ArrayList<Snotice_DTO> boardList = new ArrayList<Snotice_DTO>();
		
		String sql = "select * from SNOTICE_BOARD "
				+ " where del_or_not = 1 "
				+ " order by b_idx desc";
		...
		try {
			...
			while(S_rs.next()) {
				Snotice_DTO dto = new Snotice_DTO();
	...
	public ArrayList<Snotice_DTO> getBoards(S_SearchDTO sDto){
		ArrayList<Snotice_DTO> boardList = new ArrayList<Snotice_DTO>();
	...
	public void updateRead_cnt(int b_idx) {
		...
		String sql = "update SNOTICE_BOARD set read_cnt = (read_cnt + 1) where b_idx = ?";
	...
	//하나의 게시글 조회
	public Snotice_DTO getBoard(int b_idx){
		Snotice_DTO dto = null;
		
		String sql = "select * from SNOTICE_BOARD where b_idx = ?";
	...
	//게시글 수정하기
	public int update(Snotice_DTO dto) {
		int result = 0;//실패 시의 값
		
		try {
			if(dto.getOriginfile_name() != null) {
				//파일 업로드가 이루어질 경우의 update문
				String sql = "update SNOTICE_BOARD set title=?, content=?, originfile_name=?, savefile_name=? "
						     + " where b_idx=?";
				...
				String sql = "update SNOTICE_BOARD set title=?, content=? "
					     + " where b_idx=?";
	...
	public int delete(int b_idx) {
		...
		try {
			...
			String sql = "update SNOTICE_BOARD set del_or_not=-1 "
				     + " where b_idx=?";
