...
public class Sevent_DAO extends S_DBconnection {
	
	public int insert(Sevent_DTO dto) {
		int result = 0;//실패 시의 값
		
		try {
			if(dto.getOriginfile_name() != null) {
				//파일 업로드가 이루어질 경우의 insert문
				String sql = "insert into SEVENT_BOARD (b_idx, s_idx, writer, title, content, originfile_name, savefile_name) "
				...
				String sql = "insert into SEVENT_BOARD (b_idx, s_idx, writer, title, content) "
						     + " values(board_seq.nextval, ?,?,?,?)";
	...
	//모든 게시글 조회
	public ArrayList<Sevent_DTO> getBoards(){
		ArrayList<Sevent_DTO> boardList = new ArrayList<Sevent_DTO>();
		
		String sql = "select * from SEVENT_BOARD "
				+ " where del_or_not = 1 "
				+ " order by b_idx desc";
	...
	public ArrayList<Sevent_DTO> getBoards(S_SearchDTO sDto){
		ArrayList<Sevent_DTO> boardList = new ArrayList<Sevent_DTO>();
		try {
	...
	public void updateRead_cnt(int b_idx) {
		...
		String sql = "update SEVENT_BOARD set read_cnt = (read_cnt + 1) where b_idx = ?";
	...
	//하나의 게시글 조회
	public Sevent_DTO getBoard(int b_idx){
		Sevent_DTO dto = null;
		
		String sql = "select * from SEVENT_BOARD where b_idx = ?";
	...
	//게시글 수정하기
	public int update(Sevent_DTO dto) {
		int result = 0;//실패 시의 값
		
		try {
			if(dto.getOriginfile_name() != null) {
				//파일 업로드가 이루어질 경우의 update문
				String sql = "update SEVENT_BOARD set title=?, content=?, originfile_name=?, savefile_name=? "
						     + " where b_idx=?";
				...
				String sql = "update SEVENT_BOARD set title=?, content=? "
					     + " where b_idx=?";
	...
	public int delete(int b_idx) {
		...
		try {
			...
			String sql = "update SEVENT_BOARD set del_or_not=-1 "
				     + " where b_idx=?";
